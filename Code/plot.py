...
def plot_results(epochs, time, run, ls_cross, ls_mut):
    ...
    if run == 0:
        for mut_rate in ls_mut:
            ...
            ax_fitness.plot(
                x, GA.get_results("fitness"), label=f"Mutation rate = {mut_rate}"
            )
            ax_collision.plot(
                x, GA.get_results("coll"), label=f"Mutation rate = {mut_rate}"
            )
            ax_abs_distance.plot(
                x, GA.get_results("tot_dist"), label=f"Mutation rate = {mut_rate}"
            )
            ax_rel_distance.plot(
                x, GA.get_results("rel_dist"), label=f"Mutation rate = {mut_rate}"
            )
            ax_token.plot(
                x, GA.get_results("token"), label=f"Mutation rate = {mut_rate}"
            )
    if run == 1:
        for cross_rate in ls_cross:
            print(f"USING A CROSSOVER RATE OF: {cross_rate}")
            GA = GeneticAlgorithm(
                n_robots=2 * 20, n_iter=epochs, cross_rate=cross_rate, mut_rate=1
            )  # n_robots should be an equal number for crossover to work
            ...
            ax_collision.plot(
                x, GA.get_results("coll"), label=f"Crossover rate = {cross_rate}"
            )
            ax_abs_distance.plot(
            ...
            ax_token.plot(
                x, GA.get_results("token"), label=f"Crossover rate = {cross_rate}"
            )
